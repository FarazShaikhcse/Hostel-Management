...
if(isset($_POST['login']))
{
	...
	$stmt=$mysqli->prepare("SELECT email,password,regno FROM userregistration WHERE (email=? || regNo=?) and password=? ");
	...
	if($rs)
	{
		...
		$log="insert into userLog(userId,userIp) values('$uid','$ip')";
		$mysqli->query($log);
		if($log)
		{
								...
								<div class="well row pt-2x pb-3x bk-light">
									<div class="col-md-8 col-md-offset-2">
										...
										<form action="" class="mt" method="post">
											...
											<input type="submit" name="login" class="btn btn-primary btn-block text-lg" value="Login" >
										</form>
									</div>
								</div>
