								...
								<table id="zctb" class="table table-bordered " cellspacing="0" width="100%" border="1">
									...
									<tbody>
										...
										while($row=$res->fetch_object())
										{
											if(empty($row)){
											echo '<tr>
												<td colspan="6" style="text-align:center; color:red"><h1>You have not booked a room</h1></td>
											</tr>';
										}
										else {
										$ret1="select firstName,middleName,lastName,gender,contactNo,email from userregistration where regno = ?";
										$stmt1= $mysqli->prepare($ret1) ;
										$stmt1->bind_param('s',$row->regno);
										$stmt1->execute() ;//ok
										$stmt1->bind_result($fname,$mname,$lname,$gender,$cno,$email);
										$rs=$stmt1->fetch();
										$stmt1->close();
									}
											...
											<tr>
												<td colspan="6" style="text-align:center; color:blue"><h1>Room Info</h1></td>
											</tr>
											<tr>
